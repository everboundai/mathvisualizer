// dejong.js - Module for Peter de Jong Attractor Visualization (User Fixes Applied)

export default class DeJongVisualization {
    constructor(p, controlsElement) {
        this.p = p;
        this.controls = controlsElement;
        console.log("DeJongVisualization constructor called (User Fixes Applied).");

        // Keep defaults for the reset function
        this.defaultA = 1.4;
        this.defaultB = -2.3;
        this.defaultC = 2.4;
        this.defaultD = -2.1;
        ...
        // *** FIX: Initialize state by reading initial slider values ***
        ...
        this.skipPoints = this.defaultSkipPoints; // Keep skip points fixed for now
        ...
        // *** FIX: Attach listeners using functional helper ***
        ...
        if (this.resetBtn) {
            console.log("Attaching listener to Reset button.");
            this.resetBtn.mousePressed(() => this.resetDefaults());
        } else {
            console.warn("Reset button not found for De Jong.");
        }
        ...
        // Initial setup - Update display spans to match initial state
        this.updateControlsFromState();
        this.needsRecalculation = true; // Ensure first draw recalculates
    }

    // *** FIX: Functional addListener helper ***
    addListener(element, eventType, handler) {
      if (element) {
        // Ensure handler is called with the correct 'this' context
        element[eventType](() => handler.call(this));
      } else {
          console.warn(`De Jong control element not found during listener attachment for ${eventType}.`);
      }
    }

    updateControlsFromState() {
        console.log("Updating De Jong controls from state.");
        // Update sliders (redundant if state *came* from sliders, but needed for reset)
    ...
    updateParams() {
        ...
        // Read parameters FROM SLIDERS (source of truth for user input)
        ...
        if (newA !== this.a || newB !== this.b || newC !== this.c || newD !== this.d) {
             this.a = newA; this.b = newB; this.c = newC; this.d = newD; changed = true;
             console.log(`De Jong parameters changed: a=${this.a}, b=${this.b}, c=${this.c}, d=${this.d}`);
        }
        if (newNumPoints !== this.numPoints) { this.numPoints = newNumPoints; pointsChanged = true; console.log("De Jong numPoints changed."); }

        // *** Update display spans immediately after reading sliders ***
        if (this.valA) this.valA.html(this.a.toFixed(3)); if (this.valB) this.valB.html(this.b.toFixed(3));
        if (this.valC) this.valC.html(this.c.toFixed(3)); if (this.valD) this.valD.html(this.d.toFixed(3));
        if (this.valPoints) this.valPoints.html(this.numPoints);
    ...
    resetDefaults() {
        ...
        // Update sliders and display values from new state
        this.updateControlsFromState();
    ...
    recalculate() { /* ... calculation logic as before (skip points, calc bounds, fixed scale, draw to buffer) ... */
        ...
        let x = 0.1, y = 0.1; // *** Use slightly different initial conditions ***
        ...
        this.buffer.strokeWeight(2); let hueOffset = p.frameCount * 0.1;
