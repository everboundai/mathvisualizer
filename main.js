...
const sketch = (p) => {
    ...
    const explanations = { /* ... Keep full explanations object from previous version ... */
    ...
    p.setup = () => {
        ...
        // Specific listeners are attached inside module constructors
    ...
    function updateGeneralParams() { /* ... (Identical) ... */
        ...
        const activeViz = visualizations[currentVizKey];
        if ((activeViz?.isAnimatable ? activeViz.isAnimatable() : false)) { // Redraw if animatable changed state
             p.redraw();
        }
    }
    ...
    function visualizationChanged() { // ... (Identical - updates header/explanation, shows/hides controls/animation, sets flags) ...
        if (!vizSelect) return;
        currentVizKey = vizSelect.value();
        ...
        for (const key in visualizations) { if(visualizations[key]?.hideControls) visualizations[key].hideControls(); } // Check existence
        ...
        // Set flags ONLY when switching TO the viz
