...
const sketch = (p) => {
    ...
    const vizDisplayNames = { about: "About", flower: "Polar Flower", lorenz: "Lorenz Attractor", lissajous: "Lissajous Curves", lsystem: "L-System Generator" };
    const formulas = { about: `Select a visualization...`, flower: `r = <span class="variable">a</span> &sdot; cos(<span class="variable">k</span> &sdot; &theta;)`, lorenz: `dx/dt = &sigma;(y-x), dy/dt = x(&rho;-z)-y, dz/dt = xy-&beta;z <span class='comment'>(2D Projection)</span>`, lissajous: `x = A&sdot;sin(a&sdot;t + &delta;), y = B&sdot;sin(b&sdot;t)`, lsystem: `L-System Turtle Graphics` };
    const explanations = { // *** CORRECTED L-SYSTEM EXPLANATION ***
    ...
    p.setup = () => { // ... (Setup identical - gets elements, instantiates viz, attaches listeners) ...
        ...
        // Specific listeners are in modules

        updateGeneralParams(); // Set initial general state
        visualizationChanged(); // Set initial viz state AFTER general state is set
    ...
    function updateGeneralParams() {
        ...
        // Redraw if animation enabled *for an animatable viz*
        const activeViz = visualizations[currentVizKey];
        if (animateViz && (activeViz?.isAnimatable ? activeViz.isAnimatable() : false)) {
             p.redraw();
        }
    }
    ...
    function visualizationChanged() {
        ...
        if (animateCheckbox) {
            animateCheckbox.checked(false); // Always uncheck on switch
        }
        animateViz = false; // Always reset state variable
        ...
        if (activeViz && typeof activeViz.showControls === 'function') {
            activeViz.showControls();
             if(typeof activeViz.activate === 'function') { activeViz.activate(); } // Activate (e.g., flag recalc)
        } else if (currentVizKey !== 'about') { console.warn("No specific controls found/show method for:", currentVizKey); }

        // No longer need to set flags here, activate() handles it
        ...
        p.redraw(); // Always request redraw
    }
