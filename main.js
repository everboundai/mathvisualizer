...
import LissajousVisualization from './lissajous.js'; // Import Lissajous
...
const sketch = (p) => {
    ...
    // --- Content Maps (Ensure keys match dropdown values) ---
    const vizDisplayNames = {
        about: "About", flower: "Polar Flower", lorenz: "Lorenz Attractor",
        lissajous: "Lissajous Curves" // Added Lissajous
    };
    const formulas = {
        ...
        lissajous: `x = A&sdot;sin(a&sdot;t + &delta;), y = B&sdot;sin(b&sdot;t)` // Added Lissajous
    };
    const explanations = {
         about: `Created by Joshua Prull, 2025. Select a visualization from the dropdown.`, // Updated About
    ...
    p.setup = () => { // ... (Canvas creation identical) ...
        ...
        try {
             ...
             visualizations['lissajous'] = new LissajousVisualization(p, p.select('#lissajousControls')); // Instantiate Lissajous
        ...
        // NOTE: Specific listeners are now attached inside each module's constructor
    ...
    p.draw = () => { // ... (Draw loop identical - calls activeViz.draw()) ...
        ...
        if (activeViz?.draw) { // Check if draw function exists
            ...
            p.text("Select a visualization.", p.width / 2, p.height / 2); p.pop();
             ...
             console.warn("No active drawing function found for:", currentVizKey); // Log if viz not found
             p.push(); p.fill(255,0,100); p.textAlign(p.CENTER, p.CENTER); p.textSize(18);
             p.text(`Error: Viz "${currentVizKey}" not found.`, p.width / 2, p.height / 2); p.pop();
        }
    };
    ...
    function updateGeneralParams() { /* ... (Identical) ... */
        ...
        const activeViz = visualizations[currentVizKey];
        if (animateViz && (activeViz?.isAnimatable ? activeViz.isAnimatable() : false)) { p.redraw(); }
        // Also redraw if animation is turned OFF for an animatable viz
        else if (!animateViz && (activeViz?.isAnimatable ? activeViz.isAnimatable() : false)) { p.redraw(); }
    }


    // Handles Dropdown Change
    function visualizationChanged() { // Updated to handle animation controls display logic
        if (!vizSelect) return;
        currentVizKey = vizSelect.value();
        ...
        // Update Explanation
        const explanationHTML = activeViz?.getExplanation ? activeViz.getExplanation() : (currentVizKey === 'about' ? explanations['about'] : "");
        if (explanationContainerDiv) {
             try { explanationContainerDiv.elt.innerHTML = explanationHTML; explanationContainerDiv.style('display', (currentVizKey === 'about' || !explanationHTML) ? 'none' : 'block'); }
        ...
        // *** Update: Show if flower OR lissajous ***
        ...
        // Uncheck box only if hiding controls AND it was previously checked
        if (!showAnim && animateCheckbox?.checked()) {
