...
// import LissajousVisualization from './lissajous.js'; // Example if added back
...
const sketch = (p) => {
    ...
    const explanations = {
        about: "", // No explanation needed for about
        ...
        lorenz: `<h3>Lorenz Attractor</h3>
         ...
         // Keep Lissajous explanation if it was added and working, otherwise remove or use placeholder
         // lissajous: `<h3>Lissajous Curves</h3><p>These curves describe...</p>...`
    };
    // Ensure these maps also only contain keys for existing visualizations
     const vizDisplayNames = { about: "About", flower: "Polar Flower", lorenz: "Lorenz Attractor" };
     const formulas = { about: `Select a visualization...`, flower: `r = <span class="variable">a</span> &sdot; cos(<span class="variable">k</span> &sdot; &theta;)`, lorenz: `dx/dt = &sigma;(y-x), dy/dt = x(&rho;-z)-y, dz/dt = xy-&beta;z <span class='comment'>(2D Projection)</span>` };
    ...
    p.setup = () => { // ... (Setup identical - gets elements, instantiates viz, attaches listeners) ...
        ...
        // NOTE: Specific control listeners are now set up within the module constructors
    ...
    p.draw = () => { // ... (Draw loop identical - calls activeViz.draw()) ...
        ...
        if (activeViz && typeof activeViz.draw === 'function') {
            const is2D = (currentVizKey === 'flower' || currentVizKey === 'lissajous'); // Add other 2D keys here if needed
    ...
    function updateGeneralParams() { /* ... (Identical) ... */
        ...
        const activeViz = visualizations[currentVizKey];
        // Only redraw if the active viz is actually animatable
    ...
    function visualizationChanged() { // Uses .elt.innerHTML for explanation
        if (!vizSelect) return;
        ...
        const activeViz = visualizations[currentVizKey];
        // Use maps defined at the top level of the sketch
        ...
        // Update Explanation using the full text from the explanations object
        const explanationHTML = explanations[currentVizKey] || ""; // Get full HTML string
