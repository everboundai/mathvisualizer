...
// Helper for presets
const presets = { /* ... (keep presets object identical) ... */
...
export default class LSystemVisualization {
    constructor(p, controlsElement) {
        ...
        this.restartBtn = p.select('#restartLSystemBtn', this.controls?.elt); // Get button ref
        ...
        else { console.warn("Restart L-System Button not found"); }
    ...
    addListener(element, eventType) { /* ... (Identical) ... */ if (element) { element[eventType](() => this.updateParams()); } else { console.warn("LSystem listener element not found for", eventType); } }
    updateParams() { /* ... (Identical - Reads sliders, sets needsGeneration flag) ... */
        let presetChanged = false; let needsRegen = false;
        if (this.presetSelect) { let newPresetKey = this.presetSelect.value(); if (newPresetKey !== this.ruleSetKey && presets[newPresetKey]) { this.ruleSetKey = newPresetKey; this.angle = presets[this.ruleSetKey].angle; presetChanged = true; needsRegen = true; console.log("Preset changed:", this.ruleSetKey, "Setting angle to:", this.angle); } }
    ...
    updateControls() { /* ... (Identical - Updates slider visuals from state) ... */ if (this.iterationsSlider) this.iterationsSlider.value(this.iterations); if (this.iterationsValSpan) this.iterationsValSpan.html(this.iterations); if (this.angleSlider) this.angleSlider.value(this.angle); if (this.angleValSpan) this.angleValSpan.html(this.angle.toFixed(0)); if (this.stepLengthSlider) this.stepLengthSlider.value(this.stepLength); if (this.stepLengthValSpan) this.stepLengthValSpan.html(this.stepLength.toFixed(0)); if (this.presetSelect) this.presetSelect.value(this.ruleSetKey); }
    restart() { /* ... (Identical - flags regen & resets animation) ... */ console.log("Regenerating/Restarting L-System"); this.needsGeneration = true; this.resetAnimation(); this.p.redraw(); }
    resetAnimation() { this.currentIndex = 0; }
    generate() { /* ... (Identical - generates lstring) ... */
        console.log(`Generating L-System: ${this.ruleSetKey}, Iterations: ${this.iterations}`); const preset = presets[this.ruleSetKey]; if (!preset) { console.error("Invalid preset key:", this.ruleSetKey); this.lstring = ""; this.needsGeneration = false; return; } let current = preset.axiom; for (let i = 0; i < this.iterations; i++) { let next = ""; for (let char of current) { next += preset.rules[char] || char; } current = next; if (current.length > 500000) { console.warn("L-System length limit exceeded", i+1); this.iterations = i + 1; this.updateControls(); break; } } this.lstring = current; this.needsGeneration = false; this.resetAnimation(); console.log("L-System generated. Length:", this.lstring.length);
    }
    draw(sharedState) { /* ... (Identical - reads lstring, draws turtle based on animate state) ... */
        const p = this.p; if (this.needsGeneration) { this.generate(); } if (!this.lstring) return;
        p.push(); const preset = presets[this.ruleSetKey] || presets.plant; let startX = 0; let startY = p.height * preset.initialTranslateY; let startAngle = preset.startAngle; p.translate(startX, startY); p.rotate(startAngle);
        p.stroke(0, 0, 100); p.strokeWeight(1); let drawToIndex; let effectiveLength = this.stepLength; if (preset.lenShrink && preset.lenShrink > 1) { effectiveLength /= (preset.lenShrink ** this.iterations); } effectiveLength = Math.max(0.5, effectiveLength);
        ...
        for (let i = 0; i < drawToIndex; i++) {
            let char = this.lstring[i]; switch (char) { case 'F': case 'G': p.line(0, 0, 0, -effectiveLength); p.translate(0, -effectiveLength); break; case 'f': p.translate(0, -effectiveLength); break; case '+': p.rotate(-this.angle); break; case '-': p.rotate(this.angle); break; case '[': p.push(); break; case ']': p.pop(); break; }
        } p.pop();
    }
